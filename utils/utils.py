...
def evaluate(
    ...
    for l in range(1, pr):
        ...
        for i, trend in enumerate(real_trends):
            color = (
                "lightgreen" if trend == 1 else "pink" if trend == -1 else "lightgray"
            )
        ...
        if l % 2 == 1:
            plt.savefig(
                os.path.join(folderpath, f"{dataname}_{classname}_pred_{l}.png"),
                dpi=280,
            )
...
def rerun_AI_until_criterion_met(
    mean_accur=0.55, min_accur=0.3, max_accur=0.6, max_attempt=10
):
    def decorator_runAI(f):
        def wrapper(*args, **kwargs):
            ...
            while attempts < max_attempt:
                ...
                if (
                    sum(res) / len(res) > mean_accur
                    and min(res) > min_accur
                    and max(res) > max_accur
                ):
            ...
            while True:
                try:
                    ...
                    if x == retries:
                        ...
                        sleep_ms = backoff_in_ms * 2**x + random.uniform(0.5, 1)
