...
def trainAI(folderpath = os.path.join(os.getcwd(), 'data'), outputpath = os.path.join(os.getcwd(), 'outputs'), dataname = "GC=F_com_disagg", model = StockPredictor3):
    ...
    datapath = os.path.join(folderpath, dataname + extension)
    final_data_path = os.path.join(outputpath, dataname + postfix_2 + extension)
    model_save_path = os.path.join(outputpath, dataname + postfix_3 + ".pt")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("device:", device)

    df = extract_data(datasoursepath = datapath, finalextracteddatapath = os.path.join(folderpath, dataname + postfix_1 + extension), nCorrTop=100, nMICTop= 70)
