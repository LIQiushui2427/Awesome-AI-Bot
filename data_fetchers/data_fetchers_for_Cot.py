...
def yahoo_download(yf_code: str, start_date: dt.datetime, end_date: dt.datetime) -> pd.DataFrame:
    ...
    try:
        ...
        print("Error: ", e, ", it is possiblely because today's data is not available yet. Skip and just rename the temp file.")
        return None
    
def fetch_yf_cftc(yf_code: str, mode: str,start_date:dt, end_date:dt, hot_load:bool=False):
    ...
    if cftc_market_code is not None: # add mode to output file name
        ...
        temp_path = os.path.join(output_dir, 'temp.txt')
        end_file = os.path.join(output_dir, os.path.join(output_dir, f'{yf_code}_{end_date_}.csv'))
        start_file = os.path.join(output_dir, os.path.join(output_dir, f'{yf_code}_{start_date_}.csv'))
        
    assert isinstance(start_date, dt.datetime), 'start_date should be type of dt.datetime.'
    assert isinstance(end_date, dt.datetime), 'end_date should be type of dt.datetime.'
    assert start_date >= dt.datetime(2010,1,1), 'Do not support data fetch earlier than 2010-01-01.'
    assert end_date <= dt.datetime.today(), 'The end date should be no later than today.'
    assert start_date <= end_date, 'The start date should be no later than the end date.'
    ...
    if cftc_market_code is not None:
        ...
        cftc_df = cftc_df[(cftc_df["CFTC_Contract_Market_Code"] == cftc_market_code) & (cftc_df["Report_Date_as_YYYY-MM-DD"] >= start_date_)\
