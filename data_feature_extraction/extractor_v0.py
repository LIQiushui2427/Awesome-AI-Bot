...
def extract_data(datasoursepath, finalextracteddatapath, nCorrTop=50, nMICTop=20):
    ...
    columns_to_drop = df.select_dtypes(include=["int64", "float64"]).columns[
    ...
    correlation_with_close = (
        df.select_dtypes(include=["int64", "float64"]).corr().loc[:, "Close"].abs()
    )

    columns_after_volume = numeric_columns[numeric_columns.get_loc("Volume") + 1 :]

    top_correlated_columns = (
        correlation_with_close[columns_after_volume].nlargest(nCorrTop).index
    )
