...
def create_dataset(scaler: MinMaxScaler, df: pd.DataFrame, l, pr):
    ...
    for i in range(len(df) - l - pr + 1):
        ...
        Y.append(
            df.iloc[i + l : i + l + pr]["Close"].values
        )  # Get the closing price for the 16th day
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % early_stop[1] == 5 and epoch >= early_stop[0]:
            ...
            if len(val_accur) > 1 and val_accur[-1] > best_loss:
                print(
                    f"Validation loss: {val_accur[-1]} is not decreasing, early stop toleration: {early_stop_toleration}"
                )
                early_stop_toleration += 1
                if early_stop_toleration >= max_early_stop_toleration:
                    print(
                        f"Early stop at epoch {epoch} after {early_stop_toleration} times of toleration."
                    )
                    if logger is not None:
                        logger.info(
                            "Early stop at epoch {epoch} after {early_stop_toleration} times of toleration."
                        )
                    break
            else:
...
@rerun_AI_until_criterion_met(
    mean_accur=0.53, min_accur=0.35, max_accur=0.57, max_attempt=12
)
def trainAI(
    ticker="GC=F",
    mode="com_disagg",
    ...
    logger = get_log(
        os.path.join(os.getcwd(), "outputsFromTraining", dataname + ".log")
    )
    print(
        "AI Saving logs to:", os.path.join(os.getcwd(), "outputsFromTraining", dataname)
    )
    ...
    if os.path.exists(final_data_path):
        print(
            "Have already processed data for",
            ticker,
            "in mode",
            mode,
            "until",
    ...
    train_data = TensorDataset(
        torch.from_numpy(train_X).to(torch.float32),
        torch.from_numpy(train_Y).to(torch.float32),
    )
    val_data = TensorDataset(
        torch.from_numpy(val_X).to(torch.float32),
        torch.from_numpy(val_Y).to(torch.float32),
    )
    test_data = TensorDataset(
        torch.from_numpy(test_X).to(torch.float32),
        torch.from_numpy(test_Y).to(torch.float32),
    )
    # Create DataLoaders
    ...
    for i in range(1, pr):
        ...
        temp_df = pd.DataFrame(
            np.append(np.array(pred[i:, 0]) - np.array(pred[:-i, i]), np.nan)
        )
    ...
    for i in range(1, pr):
        bt_df.rename(
            columns={bt_df.columns[-i]: "Predict_" + str(pr - i)}, inplace=True
        )
...
if __name__ == "__main__":
    trainAI(
        ticker="GC=F", mode="com_disagg", end_date="2023-08-29", model=StockPredictor3
    )
    trainAI(
        ticker="CL=F", mode="com_disagg", end_date="2023-08-29", model=StockPredictor3
    )
