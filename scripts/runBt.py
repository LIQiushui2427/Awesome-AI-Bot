...
def runBt(datapath: str, ticker: str, mode: str, end_date: str):
    ...
    output_file = (
        os.path.join(output_path, f"{ticker}_{mode}_{end_date}.txt")
        if mode != ""
        else os.path.join(output_path, f"{ticker}_{end_date}.txt")
    )
    ...
    figs = (
        saveplots(
            ...
            file_path=os.path.join(
                output_path,
                f"{ticker}_{mode}_{end_date}.png",
            ),
            dpi=720,
            style="bar",
            barup="green",
        ...
        if mode != ""
            ...
            cerebro=cerebro,
            file_path=os.path.join(output_path, f"{ticker}_{end_date}.png"),
            dpi=720,
            style="bar",
            barup="green",
            bardown="red",
...
if __name__ == "__main__":
    ...
    runBt(
        datapath=os.path.join(
            os.path.join(os.getcwd(), "outputsByAI"), "^GSPC_fut_fin_2023-08-29.csv"
        ),
        ticker="^GSPC",
        mode="fut_fin",
        end_date="2023-08-29",
    ...
    runBt(
        datapath=os.path.join(
            os.path.join(os.getcwd(), "outputsByAI"), "GC=F_com_disagg_2023-08-29.csv"
        ),
        ticker="GC=F",
        mode="com_disagg",
        end_date="2023-08-29",
    ...
    processedDataByAI = os.path.join(
        os.path.join(os.getcwd(), "outputsByAI"), "AAPL_2023-08-29.csv"
    )
    get_signals(file_path=processedDataByAI)
    runBt(datapath=processedDataByAI, ticker="AAPL", mode="", end_date="2023-08-29")

    processedDataByAI = os.path.join(
        os.path.join(os.getcwd(), "outputsByAI"), "^HSCE_2023-08-29.csv"
    )
    get_signals(file_path=processedDataByAI)
    runBt(datapath=processedDataByAI, ticker="^HSCE", mode="", end_date="2023-08-29")
    ...
    processedDataByAI = os.path.join(
        os.path.join(os.getcwd(), "outputsByAI"), "0388.HK_2023-08-29.csv"
    )
    get_signals(file_path=processedDataByAI)
    runBt(datapath=processedDataByAI, ticker="0388.HK", mode="", end_date="2023-08-29")
    ...
    processedDataByAI = os.path.join(
        os.path.join(os.getcwd(), "outputsByAI"), "^HSI_2023-08-29.csv"
    )
    get_signals(file_path=processedDataByAI)
    runBt(datapath=processedDataByAI, ticker="^HSI", mode="", end_date="2023-08-29")

    processedDataByAI = os.path.join(
        os.path.join(os.getcwd(), "outputsByAI"), "TSLA_2023-08-29.csv"
    )
    get_signals(file_path=processedDataByAI)
    runBt(datapath=processedDataByAI, ticker="TSLA", mode="", end_date="2023-08-29")
