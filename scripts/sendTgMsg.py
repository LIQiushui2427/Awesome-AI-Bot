...
class Bot:
    ...
    async def sendMsg(self, msg, parse_mode: str = "Markdown"):
        await self.bot.send_message(
            chat_id=self.chat_id, text=msg, parse_mode=parse_mode
        )
        pass

    async def sendImg(self, img_path: str, caption: str = None):
        await self.bot.send_photo(
            chat_id=self.chat_id, photo=open(img_path, "rb"), caption=caption
        )
        pass

    async def sendFile(self, file_path: str, caption: str = None):
        await self.bot.send_document(
            chat_id=self.chat_id, document=open(file_path, "rb")
        )
        pass

    async def sendMediaGroup(
        self, media: list, caption: str = None, parse_mode: str = "Markdown"
    ):
    ...
    async def sendSticker(self, sticker_name: str):
        await self.bot.send_sticker(
            chat_id=self.chat_id, sticker=STICKERS[sticker_name]
        )
        pass
...
@retry_with_backoff(15)
async def send_daily_ticker_report(
    bot: Bot, date: str, ticker: str, BtDict: dict, AIDict: dict, LogDict: dict
):
    ...
    last_date, size, sharpeRatio, winRate, profitFactor, trades_per_year = get_trades(
        Bt_file
    )

    if last_date is not None:
        last_trade_msg = BT_STATS.format(
            bt_start_date,
            bt_end_date,
            last_date,
            winRate,
    ...
    # else:
    msg = (
        BUY_MSG.format(TICKER_DICT[ticker], yesterday_close, date)
        if "-" not in size
        else SELL_MSG.format(TICKER_DICT[ticker], yesterday_close, date)
    )
        ...
        if ".png" in file:
            media_group.append(
                telegram.InputMediaDocument(
                    media=open(file, "rb"), caption=msg, parse_mode="Markdown"
                )
            )
    for file in AIDict[ticker]:
        if ".png" in file:
            media_group.append(
                telegram.InputMediaDocument(media=open(file, "rb"), caption="AI signal")
            )
    for file in LogDict[ticker]:
        # print(file)
        if ".png" in file:
            filename = (
                file.split(seperator)[-1].split("_")[-2]
                + "_"
                + file.split(seperator)[-1].split("_")[-1]
            )
            media_group.append(
                telegram.InputMediaDocument(
                    media=open(file, "rb"),
...
def daily(date):
    ...
    for ticker in tickers:
        asyncio.run(
            send_daily_ticker_report(bot, date, ticker, myBtDict, myAIDict, myLogDict)
        )
