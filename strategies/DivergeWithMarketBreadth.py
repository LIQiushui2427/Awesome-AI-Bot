...
class LookMarketBreadthStrategy(SmaCross):
    ...
    def init(self):
        ...
        self.market_breadth_larger_sma50 = self.data.df['constituent-sma50_larger_price']
        self.market_breadth_smaller_sma50 = self.data.df['constituent-sma50_smaller_price']
        self.market_sma_50 = self.data.df['market-sma50_smaller_price'] / (self.data.df['market-sma50_larger_price'] + self.data.df['market-sma50_smaller_price'])
...
class DivergenceStrategy(SignalStrategy, TrailingStrategy):
    ...
    def init(self):
        ...
        self.divergence[(self.rsi.shift() < 100 - self.threshold) & (self.market_sma_50 > self.market_sma_50.shift())] = 1
        self.divergence[(self.rsi.shift() > self.threshold) & (self.market_sma_50 < self.market_sma_50.shift())] = -1
