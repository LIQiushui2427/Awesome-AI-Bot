...
class BOLLStrategy(SignalStrategy):
    ...
    def init(self):
        ...
        self.bollinger = pta.bbands(
            self.data.df["Close"], length=self.bollinger_period, std=self.bollinger_std
        )
        ...
        self.sell_sig = self.I(
            lambda x, y: x > y, self.data.Close, self.Bollinger_Lower
        )
